...
- 配置中新增 `"course_sort_enabled": "1"`（或在Web页面勾选`程序运行时自动排序课程`）后，程序在获取课程信息后会自动对课程执行顺序做拓扑排序：**退课先于与其冲突的选课执行**（同一时间段、周次重叠，或同一课程号的不同教学班），其余选课尽量靠前抢课。
- 选课之间按开课类型优先（`体育分项` > `通识选修课` > 其他，此类课程通常竞争最激烈），同优先级保持原有相对顺序。
- 不开启时完全保持配置文件中的手动顺序；Web页面也提供`自动排序`按钮（仅调整页面上的顺序，保存后生效）和每行`↑` `↓`按钮手动微调。

## Web课程名显示

- Web配置页面会根据课程编号自动显示课程名称与上课时间，便于手动调整课程顺序。
- 课程信息来源：本地`course.json`或`任务落实情况课程导出.xlsx`（与配置的学年学期对应）。首次使用请先运行一次程序获取课程信息。

## ICS日历导出

- 配置中设置 `"semester_start_date": "YYYY-MM-DD"`（**第1周星期一**的日期）后，程序会在**抢课完成后**在线拉取个人完整课表并生成 `timetable_<学年学期>_full.ics`，可导入Apple日历、Google日历等。
- **在抢课后生成**意味着日历反映实际选课结果：自动排除未选上的备选教学班（同一课程配置多个备选班时无需担心日历重复），并包含**教务处预置分配、无需手动选课的课程**，是真正完整的个人课表。
- 复用程序运行时的登录会话请求教务系统 `xskbcx_cxXsgrkb` 接口，**不需要浏览器自动化，也无需额外提供账号密码**。
- 因部分课程单双周上课，**每次上课生成一条独立日历事件**，不使用重复规则。
- 在线获取失败时，回退为按选课配置(course.json/Excel)生成 `timetable_<学年学期>.ics`。
- 节次作息时间见`pkg/course/ics.go`中的`PeriodTimes`，如与校区作息不符请自行调整。
- Web配置页也提供按选课配置导出ICS的下载按钮(登录前预览用)。

### 关于第1周星期一日期

- 教务系统不提供“第1周星期一”的日历日期（课表接口只给出各门课的周次范围 `zcd`，没有日历锚点），因此该日期需在配置中**手动填写一次**（每学期一次，可查 HDU 校历）。这与主流正方教务适配器的做法一致。
- 若未填写，程序会跳过ICS生成并给出提示，其余选退课功能不受影响。
