...
- 配置中设置 `"semester_start_date": "YYYY-MM-DD"`（**第1周星期一**的日期）后，程序运行时会为课程列表中**选课**的课程生成ICS日历文件（`timetable_<学年学期>.ics`），可导入Apple日历、Google日历等。
- 因部分课程单双周上课，**每次上课生成一条独立日历事件**，不使用重复规则。
- 同一课程配置了多个备选教学班时，仅为顺序最靠前的教学班生成日历。
- 节次作息时间见`pkg/course/ics.go`中的`PeriodTimes`，如与校区作息不符请自行调整。
- Web页面也提供ICS的下载按钮。
- 上述 `timetable_<学年学期>.ics` 仅包含配置中参与选课的课程。

### 个人完整课表

- 设置 `semester_start_date` 后，程序在**登录成功**后还会在线拉取个人完整课表（复用已有登录态请求教务系统 `xskbcx_cxXsgrkb` 接口），生成 `timetable_<学年学期>_full.ics`。
- 与上一个文件不同，该课表包含**教务处预置分配、无需手动选课的课程**，是真正完整的个人课表；同样每次上课为独立事件，支持单双周。
- 该功能不需要浏览器自动化，仅复用程序运行时的登录会话，无需额外提供账号密码。
